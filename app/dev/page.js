...
export default function Page() {
  ...
  const [options,optionDispatch]=useReducer(optionReducer,provinces,undefined)
  const openOptions = ()=>{
    console.log(
  ...
  return (
    <div className={" flex justify-around pointer-events-auto-20 p-20 w-full"}>
      <button className={'p-20 border-2'} onClick={()=>optionDispatch({type:"inc"})}>Tab up</button>
      <div className={'grid'}>
        {options.slice(0,options.length/2).map(option=>{
          return(
              <button type={'button'} key={option.id} className={' w-[200px] border '} onClick={openOptions}>{option.value}</button>
          )
        })}
        <button type={'button'} className={' w-[200px] border bg-teal-800'} onClick={openOptions}>
          {
            // console.log()
            options[Math.floor(options.length/2)].value
          }
      ...
      <button className={'p-20 border-2'} onClick={()=>optionDispatch({type:"dec"})}>Tab down</button>
